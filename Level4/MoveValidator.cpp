        ...
        Vertex destination)
{
    ...
    if (m_board[destination.y][destination.x] == 'R')
    {
        dest_is_rock = true;
    }

    // Check if destination is blocked by barrier
    if (moveIsOpen(origin, destination))
    {
        dest_is_open = true;
    }

    if (is_valid_shape && dest_on_board && !dest_is_rock && dest_is_open)
    {
        tests_passed = true;
    }
...
/* Algorithm - Check if the move is open; The move is open if the destination is
 *             not a barrier and a barrier does not lie within the path
 * 
 */
bool MoveValidator::moveIsOpen(Vertex start, Vertex end)
{
    bool move_is_open    = true;
    ...
    {
        // Destination is open; Check other two nodes in path for barrier
